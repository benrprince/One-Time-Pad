...
void decrypt(char *text, char *key, char *decryptedText) {
    ...
    for(i=0; i < strlen(text); i++) {
        ...
        // Get decrypted character int
        newChar = textChar - keyChar;
        if(newChar < 0) {
            newChar += 27;
        }
        // Convert decrypted character int to a char value
        if(newChar == 26) {
            decryptedText[i] = ' ';
        } else {
            decryptedText[i] = newChar + 'A';
        }
    }
        ...
        switch(spawnpid) {
            ...
            case 0: {
                ...
                while(charsRead < strlen(decryptedText)) {
                    charsRead += send(connectionSocket, decryptedText, 1000, 0);
                }
                if (charsRead < 0){
